...
router.get('/all', (req, res, next) => {
  pool.query('SELECT * FROM users', (q_err, q_res) => {
    if (q_err) return next(q_err); //pass the error to middleware
...
const set_session_cookie = (session_str, res) => {
  res.cookie('session_str', session_str, {
    expire: Date.now() + 3600000, //expire in an hour later
    httpOnly: true
...
const set_session = (username, res, session_id) => {
  ...
  return new Promise((resolve, reject) => {
    if (session_id) {
      ...
      pool.query('UPDATE users SET session_id = $1 WHERE username_hash = $2', [session.id, hash(username)], (q_err, q_res) => {
...
router.post('/new', (req, res, next) => {
  ...
  pool.query('SELECT * FROM users WHERE username_hash = $1', [username_hash], (q0_err, q0_res) => {
    if (q0_err) return next(q_err);

    if (q0_res.rows.length === 0) {
      //insert a new user
      pool.query('INSERT INTO users(username_hash, password_hash) VALUES($1, $2)', [username_hash, hash(password)], (q1_err, q1_res) => {
      ...
      res.status(409).json({ type: 'error', msg: 'This username has been taken' });
    }
  });
});
...
router.post('/login', (req, res, next) => {
  ...
  pool.query('SELECT * FROM users WHERE username_hash = $1', [hash(username)], (q_err, q_res) => {
...
router.get('/logout', (req, res, next) => {
  ...
  pool.query('UPDATE users SET session_id = NULL WHERE username_hash = $1', [hash(username)], (q_err, q_res) => {
    if (q_err) return next(q_err);

    res.clearCookie('session_str');

    res.json({ msg: 'Successfull logout' });
  });
});
...
router.get('/authenticated', (req, res, next) => {
  ...
  pool.query('SELECT * FROM users WHERE username_hash = $1', [hash(username)], (q_err, q_res) => {
    ...
    res.json({ authenticated: Session.verify(req.cookies.session_str) && q_res.rows[0].session_id === id });
  });
});
